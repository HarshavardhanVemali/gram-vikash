...
class TriggerCallView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not all([exotel_sid, exotel_api_key, exotel_api_token, exotel_from_number]):
            return Response({"error": "Exotel credentials not configured"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        ...
        try:
            ...
            if response.status_code == 200:
                return Response({"message": "Call initiated successfully", "sid": response.json().get('Call', {}).get('Sid')}, status=status.HTTP_200_OK)
            else:
                logger.error(f"Exotel Call Error: {response.status_code} {response.text}")
                return Response({"error": f"Failed to initiate call: {response.text}"}, status=status.HTTP_400_BAD_REQUEST)
